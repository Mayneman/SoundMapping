...
Dependencies for some applications:nmsimhlpr.py, NumPy
        ...
        while rh < rh_max:
            ...
            if model == "nmsimgis":
                ...
                absorption_lst = NMSim_atmospheric_absorption(nmsim_dll, temp, rh, freq,
                                                                    nfreq, distance)
                for i in range(len(absorption_lst)):
...
def network_analyst_helper(new_points_file, point_lst, order_id, network, analysis_dir, vehicle_speed, speed_units, start_time):
    ...
    for i in range(n_reps): #Remove [0:1] when running for all destinations
...
def output_timing(timelog, my_times, my_time_labels, delete_existing_timelog):
    ...
    if timelog != "none":
        ...
        # Create header row
        print("Task, Process Time, Elapsed Time, Percent of Total", file=t)
        ...
        for i in range(1,len(my_times)):
            ...
            print("%s,%s,%s,%.1f" % (my_time_labels[i], item_time, elapsed_time, perc_time), file=t)
...
def extract_octave_indices(field, one_third_octave_bands, one_third_octave_indices, i, old_band):
    for j in range(len(one_third_octave_bands)):
...
# Try to make an R-like sequence function (see range function)
...
def get_extent(file):
    ...
    extent_string = "%s %s %s %s" % (extent.XMin, extent.YMin, extent.XMax, extent.YMax)
